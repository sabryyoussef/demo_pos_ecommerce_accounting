...
# Do not touch pos.config here (sessions/orders reference them; renaming breaks Odoo.sh upgrades).
print("SKIP_POS_CONFIG_RENAME_SAFE_FOR_MODULE_INSTALL")

admin_pw = os.environ.get("GATEB1_ADMIN_PW")
if admin_pw:
    admin = env["res.users"].search([("login", "=", "admin")], limit=1)
    admin.write({"password": admin_pw})
    print("ADMIN_PASSWORD_ROTATED", admin.login)
else:
    print("SKIP_ADMIN_PASSWORD_ROTATE_NO_GATEB1_ADMIN_PW")
